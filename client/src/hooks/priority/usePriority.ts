...
export const usePrioritys = (condition: IConditionFilter) => {
  ...
  const query = useQuery({
    queryKey: [queryKeys.prioritys, { ...filter }],
    queryFn: findMany,
  });
  return { ...query, setFilter };
};

export const usePriority = (id: string) => {
  ...
  const query = useQuery({
    queryKey: [queryKeys.priority, filter],
    queryFn: findById,
  });
  return { ...query, setFilter };
};
