...
export const useEquipStatues = (condition: IConditionFilter) => {
  ...
  const query = useQuery({
    queryKey: [queryKeys.equip_statues, { ...filter }],
    queryFn: findMany,
  });
  return { ...query, setFilter };
};

export const useEquipStatus = (id: string) => {
  ...
  const query = useQuery({
    queryKey: [queryKeys.equip_status, filter],
    queryFn: findById,
  });
  return { ...query, setFilter };
};
