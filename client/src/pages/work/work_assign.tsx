...
import { useWorksByStatus, useWorkSubmitMutation } from "@/hooks/work";
...
import { useWorkAssignStore } from "@/stores/useWorkAssignStore";

const title = "งานซ่อมที่ได้รับมอบหมาย";
const listItems = [{ title: title, href: "#" }];

export default function WorkAssign() {
  const navigate = useNavigate();
  const workAssignStore = useWorkAssignStore();
  const [debounce] = useDebouncedValue(workAssignStore.txtSearch, 500);
  const [sortStatus, setSortStatus] = useState<DataTableSortStatus>({
    columnAccessor: workAssignStore.sortField,
    direction: workAssignStore.sortDirection,
  });

  const setCondition = () => {
    const condition = {
      txtSearch: workAssignStore.txtSearch,
      page: workAssignStore.page - 1,
      sortField: sortStatus.columnAccessor,
      sortDirection: sortStatus.direction,
    };
    return condition;
  };
  const { data, isLoading, setFilter } = useWorksByStatus(setCondition(), "3");
  ...
  const clearSearchData = () => {
    workAssignStore.setFilter({ ...workAssignStore, txtSearch: "", page: 1 });
  };
  ...
  useEffect(() => {
    searchData();
  }, [debounce, workAssignStore.page, sortStatus]);
  return (
    <>
      ...
      <Card shadow="sm">
          ...
          <Grid mx="md" mt="md" align="flex-end">
            <Grid.Col span={{ md: 4 }}>
              <InputSearch
                ...
                value={workAssignStore.txtSearch}
                onChange={(e) => {
                  workAssignStore.setFilter({
                    ...workAssignStore,
        ...
        <DataTable
          ...
          columns={[
            ...
            },
            {
              ...
              render({ name, priority_name }) {
                return (
                  <Group>
                    <Highlight highlight={workAssignStore.txtSearch} size="sm">
                      {String(name)}
                    </Highlight>
              ...
              render: ({ id, name }) => (
                <Group justify="center" gap={3} wrap="nowrap">
                  <Button
                    ...
                    color="yellow"
                    onClick={() =>
                      navigate(`/work_assign/details/${id}`, {
                        state: {
                          mainTitle: title,
                          from: "/work_assign",
                          type: "workAssign",
          ...
          page={workAssignStore.page}
          onPageChange={(p: number) =>
            workAssignStore.setFilter({ ...workAssignStore, page: p })
          }
